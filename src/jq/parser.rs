    ...
    if c.is_ascii_digit() {
      ...
      while j < n
        && (chars[j].is_ascii_digit()
          ...
          || chars[j] == '-')
      {
        if (chars[j] == '+' || chars[j] == '-') && !(chars[j - 1] == 'e' || chars[j - 1] == 'E') {
          break;
        }
        j += 1;
      }
...
impl QParser {
  ...
  fn at(&self, value: &str) -> bool {
    let token = self.peek();
    (token.type_ == QTokenType::Punct || token.type_ == QTokenType::Name) && token.value == value
  }
  ...
  fn parse_object_body(&mut self) -> QResult<QNode> {
    ...
    while !self.at("}") {
      ...
      if key.type_ != QTokenType::Name && key.type_ != QTokenType::Str {
        return Err("Only literal object keys are supported by the jq converter.".to_string());
      }
      self.next();
      if self.eat(":") {
